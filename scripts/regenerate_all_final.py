...
运行后会自动调用 speedtest.py 对 allowed_ips_with_country.txt 分组测速。
...
def is_valid_entry(s: str) -> bool:
    """
    过滤明显无效的条目。
    例如：0.5.0.0.7.0.f.1.0.7.4.0.1.0.0.2.xzhi.eu.org 这种伪IPv6+域名混合体
    """
    if not s:
        return False

    # 过滤点号过多的伪IPv6（正常域名最多4-5个点，IPv4有3个，这种有15+个）
    dot_count = s.count('.')
    colon_count = s.count(':')

    # 如果点号超过6个且没有冒号，且不是纯IP，大概率是格式错误的伪IPv6
    if dot_count > 6 and colon_count == 0:
        # 进一步检查：是否像 0.x.x.x.x.x.x.x.x.x.x.x.x.x.x.domain.com
        parts = s.split('.')
        # 如果前面很多部分是单个十六进制字符（0-9, a-f），就是伪IPv6
        hex_like_count = sum(1 for p in parts if len(p) == 1 and p.lower() in '0123456789abcdef')
        if hex_like_count >= 8:
            print(f"  [Filter] Skipping invalid pseudo-IPv6 entry: {s[:60]}...")
            return False
    ...
    # 过滤过长的条目（正常IP或域名不会太长）
    if len(s) > 253:  # DNS最大长度
        print(f"  [Filter] Skipping overly long entry: {s[:60]}...")
        return False
...
def collect_from_manual(manual_file: Path) -> dict:
    ...
    if skipped > 0:
        print(f"  Skipped {skipped} invalid entries from {manual_file}")
...
def collect_from_urls(urls_file: Path) -> dict:
    ...
    if skipped > 0:
        print(f"  Skipped {skipped} invalid entries from {urls_file}")
    return results
...
def collect_from_domains(domains_file: Path) -> dict:
    ...
    if skipped > 0:
        print(f"  Skipped {skipped} invalid entries from {domains_file}")
    return results
